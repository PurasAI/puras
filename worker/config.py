...
class WorkerSettings(BaseSettings):
    ...
    # --- Local web tools (local mode only) ---
    # Hosted runs go through the platform's /v1/web search/fetch service. A local
    # run has no platform, so `web_fetch` runs as a direct Python HTTP GET and
    # `web_search` runs through Anthropic's server-side web_search tool on the BYO
    # ANTHROPIC_API_KEY. This is the Anthropic model id that backs the offline
    # web_search helper (any model that supports the web_search tool); BYO key, so
    # keep it cheap. Override if you want a different/cheaper model.
    local_web_search_model: str = Field(
        "claude-haiku-4-5", alias="PURAS_LOCAL_WEB_SEARCH_MODEL"
    )
